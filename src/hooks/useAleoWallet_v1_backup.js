    /**
     * Minimal test: call a contract function with a single field argument
     */
    const testSimpleFieldCall = useCallback(async (fieldValue) => {
        if (!publicKey) throw new WalletNotConnectedError();
        if (!requestTransaction) throw new Error('requestTransaction is not available');

        // Use a simple transition, e.g., test_field(field: field)
        // You must have such a function in your contract:
        // async transition test_field(input: field) -> field { return input; }
        const inputs = [String(fieldValue).replace(/\s+/g, '') + 'field'];

        const aleoTransaction = Transaction.createTransaction(
            publicKey,
            WalletAdapterNetwork.TestnetBeta,
            'privcaster_v1.aleo', // Make sure this matches your deployed contract
            'test_field', // The simple test transition
            inputs,
            10000 // minimal fee
        );
...
export const useAleoWallet = () => {
    ...
    /**
     * Serialize a JS Identity object to Aleo record string
     */
    function serializeIdentityRecord(identity) {
        // Ensure user_id ends with 'field'
        // Pass user_id as a quoted string with 'field' suffix
        let userId = identity.user_id;
        // Leo-style record string
        userId = String(userId).replace(/\s+/g, '');
        if (!userId.endsWith('field')) {
            userId = userId.replace(/[^0-9]/g, '') + 'field';
        }
        const rep = String(Number(identity.reputation_score) || 0) + 'u64';
        const followers = String(Number(identity.follower_count) || 0) + 'u64';
        const following = String(Number(identity.following_count) || 0) + 'u64';
        return `{ owner: "${identity.owner}", user_id: "${userId}", reputation_score: "${rep}", follower_count: "${followers}", following_count: "${following}" }`;
    }
    ...
    const createCast = useCallback(async (identityRecord, castIdField, contentHashField) => {
        ...
        // Serialize identity record for Aleo contract
        const identityAleoRecord = serializeIdentityRecord(identityRecord);
        console.log('[Aleo] Serialized Identity Record:', identityAleoRecord);
        const inputs = [identityAleoRecord, castIdField, contentHashField, `${timestamp}u64`];
    ...
    return {
        ...
        // Minimal test function for contract call
        testSimpleFieldCall,
